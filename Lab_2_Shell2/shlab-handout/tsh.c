...
void eval(char *cmdline) 
{
    ...
    if (!bg) {
        // Adding job (Only last child's pid)
...
void do_bgfg(char **argv) 
{
    ...
    if (strcmp(argv[0], "bg") == 0) {
        ...
        if (strstr(argv[1], "%") == NULL) {
            ...
            strcpy(temp, argv[1]);
            memcpy(jidString, &temp[1], 5);

            int jid = atoi(jidString);
            if (jid > maxjid(jobs)) {
                printf("%c%d: No such job\n", '%', jid);
                return;
            }
            pid_t pid = getjobjid(jobs, jid)->pid;
            pid_t pgid = getjobpid(jobs, pid)->pgid;

            printf("[%d] (%d) %s", jid, pid, getjobpid(jobs, pid)->cmdline);
            
            // Find a job and send SIGCONT signal
            kill(-pgid, SIGCONT);
...
void sigchld_handler(int sig) 
{
    ...
    while ((pid = waitpid(-1, &state, WUNTRACED | WNOHANG)) > 0) {
        if (WIFSTOPPED(state)) {
            ...
            //write(STDERR_FILENO, "deleting child\n", sizeof "deleting child\n"-1);
...
void sigint_handler(int sig) 
{
    // Send SIGINT signal -> terminate current fg process group
    if (fgpid(jobs) != 0) {
        //printf("here\n");
